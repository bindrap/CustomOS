...
║   ╚═╝╚══════╝ ╚═════╝     ╚═════╝  ╚═════╝ ╚═╝╚══════╝╚═════╝ ║
║                                                           ║
║        Custom Arch Linux ISO Builder (Docker)             ║
║                                                           ║
╚═══════════════════════════════════════════════════════════╝
EOF
echo -e "${NC}"
...
# Customize ISO label
# Convert to uppercase and remove dots from version
...
if [ -f "$OUTPUT_DIR"/*.iso ]; then
    ...
    echo "     - Name: CustomOS"
    echo "     - Type: Linux"
    echo "     - Version: Arch Linux (64-bit)"
    echo "     - RAM: 4GB (minimum 2GB)"
    echo "     - Disk: 40GB+"
    echo ""
    echo "  3. In VM Settings → Storage:"
    echo "     - Click 'Empty' CD drive"
    echo "     - Click disk icon → Choose disk file"
    echo "     - Select: $WIN_PATH"
    echo ""
    echo "  4. Start VM and run: install-arch"
    echo ""
    echo "  5. Answer prompts and wait 15-20 minutes"
    echo ""
    echo "  6. Reboot into your fully configured Arch Linux!"
    echo ""
else
    echo ""
    echo -e "${RED}╔═══════════════════════════════════════════════════════════╗${NC}"
    ...
    echo ""
    echo "Solutions:"
    echo "  1. Try running ./build-iso-docker.sh again (will use faster mirrors)"
