...
# CustomOS Jan12 ISO Builder
# Lightweight XFCE edition for 4GB RAM laptops
...
===================================================
  ...
  XFCE Lightweight Edition ISO Builder
===================================================
EOF
echo -e "${NC}"

ISO_NAME="pbos-xfce-lite"
...
echo ""
echo -e "${YELLOW}PBOS (Parteek Bindra Operating System) - XFCE Lightweight Edition${NC}"
echo "Output: $OUTPUT_DIR/${ISO_NAME}-${ISO_VERSION}.iso"
echo ""
echo "Optimized for:"
echo "  ✓ 4GB RAM laptops"
echo "  ✓ Lightweight XFCE desktop"
echo "  ✓ Gaming on low-spec hardware"
echo "  ✓ Fast boot and low resource usage"
echo ""
...
WORK_DIR="/tmp/archiso-customos-jan12"
ISO_DIR="$WORK_DIR/iso-build"
OUTPUT_DIR="/workspace/cos_jan12/iso-output"
...
if [ -f "/workspace/cos_jan12/install-auto.sh" ]; then
    cp /workspace/cos_jan12/install-auto.sh airootfs/root/custom-setup/
...
if [ -f "/workspace/cos_jan12/install.sh" ]; then
    cp /workspace/cos_jan12/install.sh airootfs/root/custom-setup/
...
if [ -f "/workspace/cos_jan12/post-install.sh" ]; then
    cp /workspace/cos_jan12/post-install.sh airootfs/root/custom-setup/
...
if [ -f "/workspace/cos_jan12/wifi-setup.sh" ]; then
    cp /workspace/cos_jan12/wifi-setup.sh airootfs/root/custom-setup/
...
if [ -f "/workspace/cos_jan12/partition-helper-safe.sh" ]; then
    cp /workspace/cos_jan12/partition-helper-safe.sh airootfs/root/custom-setup/
...
if [ -f "/workspace/cos_jan12/SAFE_DUAL_BOOT_SETUP.md" ]; then
    cp /workspace/cos_jan12/SAFE_DUAL_BOOT_SETUP.md airootfs/root/custom-setup/
    echo "  ✓ SAFE_DUAL_BOOT_SETUP.md copied"
fi
if [ -f "/workspace/cos_jan12/DUAL_BOOT_GUIDE.md" ]; then
    cp /workspace/cos_jan12/DUAL_BOOT_GUIDE.md airootfs/root/custom-setup/
    echo "  ✓ DUAL_BOOT_GUIDE.md copied"
fi
if [ -f "/workspace/cos_jan12/SWAP_SETUP_GUIDE.md" ]; then
    cp /workspace/cos_jan12/SWAP_SETUP_GUIDE.md airootfs/root/custom-setup/
    echo "  ✓ SWAP_SETUP_GUIDE.md copied"
fi

# Copy stoic quotes
if [ -f "/workspace/cos_jan12/stoic-quotes.txt" ]; then
    cp /workspace/cos_jan12/stoic-quotes.txt airootfs/root/custom-setup/
    mkdir -p airootfs/usr/share/pbos
    cp /workspace/cos_jan12/stoic-quotes.txt airootfs/usr/share/pbos/
    echo "  ✓ stoic-quotes.txt copied"
fi

# Copy MOTD generator script
if [ -f "/workspace/cos_jan12/generate-motd.sh" ]; then
    cp /workspace/cos_jan12/generate-motd.sh airootfs/usr/local/bin/
    chmod +x airootfs/usr/local/bin/generate-motd.sh
    echo "  ✓ generate-motd.sh copied"
fi

# Copy issue generator script
if [ -f "/workspace/cos_jan12/generate-issue.sh" ]; then
    cp /workspace/cos_jan12/generate-issue.sh airootfs/usr/local/bin/
...
XFCE Lightweight Edition - Optimized for 4GB RAM Laptops
...
# Generate /etc/issue with PBOS branding
...
echo ""
echo "=================================================="
echo "  Building PBOS XFCE Lite ISO - 10-15 minutes"
echo "=================================================="
echo ""
...
if [ -f "$ISO_FILE" ]; then
    ...
    echo ""
    echo -e "${GREEN}====================================================${NC}"
    echo -e "${GREEN}  PBOS XFCE Lite ISO Build Complete!${NC}"
    echo -e "${GREEN}====================================================${NC}"
    echo ""
    ...
    echo "Features:"
    echo "  ✓ Lightweight XFCE desktop"
    echo "  ✓ Optimized for 4GB RAM laptops"
    echo "  ✓ Gaming support (Steam, Wine)"
    echo "  ✓ WiFi and partition helper scripts"
    echo "  ✓ Dual boot safe installation"
    echo "  ✓ Zram swap for better performance"
    echo ""
    echo "Test with QEMU:"
